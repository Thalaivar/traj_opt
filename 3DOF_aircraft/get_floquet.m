function [eig_vec, eig_val, sol] = get_floquet(aircraft, N, M, xguess)
    ...
    if isempty(xguess)
        xguess = [(1/sqrt(12))*ones(2*M*n,1);0.02;0.0204];
    end
    ...
    options = optimoptions('fmincon', 'Display', 'Iter', 'Algorithm', 'interior-point', 'MaxFunctionEvaluations', 1000000, 'ConstraintTolerance', 1e-3);
