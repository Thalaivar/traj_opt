...
function [c, ceq, dc, dceq] = constFun(X, aircraft, N, M, floq)
    ...
    % if eigenvalues are being estimated
       ...
       eigvec_comp = [X(1:6*M,1), X(6*M+1:2*6*M,1)]; eigval = complex(X(end-1,1), X(end,1));
       ...
       alpha = zeros(6,M); beta = zeros(6,M);
       for i = 1:M
           j = (i-1)*6 + 1;
           alpha(:,i) = eigvec_comp(j:j+5,1);
           beta(:,i)  = eigvec_comp(j:j+5,2);
       end
       ...
       diffmat = [D       ,zeros(M),zeros(M),zeros(M),zeros(M),zeros(M);
                  zeros(M),D       ,zeros(M),zeros(M),zeros(M),zeros(M);
                  zeros(M),zeros(M),D       ,zeros(M),zeros(M),zeros(M);
                  zeros(M),zeros(M),zeros(M),D       ,zeros(M),zeros(M);
                  zeros(M),zeros(M),zeros(M),zeros(M),D       ,zeros(M);
                  zeros(M),zeros(M),zeros(M),zeros(M),zeros(M),D       ];
              
       % vector of the form [u1', u2', .. , u6']', where ui is 
       % a column vector of  the value of i'th component of the eigenvector
       % at M points
       udot_real = zeros(6*M,1); udot_img = zeros(6*M,1);
       for i = 1:6
           j = (i-1)*M+1;
       ...
       udot_real_actual = zeros(M,6); udot_img_actual = zeros(M,6);
       for i = 1:M
           I = eye(6);
       ...
       alphadot = zeros(6*M,1); betadot = zeros(6*M,1);
       for i = 1:6
           j = (i-1)*M+1;
