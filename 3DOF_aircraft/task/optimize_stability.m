...
function [aircraft, sol] = optimize_stability(aircraft, x0, p, M, params)    
...
%     lb(end,1) = 0; ub(end,1) = 150;
    ...
    sol = fmincon(@(x) objfun(x, aircraft, 'floq_new', params), xguess, [], [], [], [], lb, ub, @(x) constFun_traj(x, aircraft, 'traj', M), options);
