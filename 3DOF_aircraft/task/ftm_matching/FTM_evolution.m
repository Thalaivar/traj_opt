...
load('D:\ranjth_mohan\traj_opt\3DOF_aircraft\solutions\stability_opt\expo_O_stable_fin.mat')

%% time evolution
options = odeset('AbsTol', 1e-11, 'RelTol', 1e-11, 'Events', @(t,y) z_event(t,y));
data = simulate_traj(ac, 200, options);
    
%% evolution of FTM over all orbits
t = data.t; y = data.y;
n_max = floor(t(end)/ac.tf);
FTM_diff = zeros(1, n_max-1);
% expo, time-march-ode-aug, time-march-fs,  
type = 'friedmann';
for i = 1:n_max-1
    [~,j] = min(abs(t-i*ac.tf));
    [~,k] = min(abs(t - (i+1)*ac.tf));
    if(i == 1), FTM_nom = retrieve_FTM(ac, t, y, [1,j], type); end
    FTM = retrieve_FTM(ac, t, y, [j,k], type);
    FTM_diff(i) = norm(FTM-FTM_nom)/norm(FTM_nom);
end
...
%% get FTM for an orbit
function FTM = retrieve_FTM(ac, t, y, span, type)
    j = span(1); k = span(2);
    switch(type)
        case 'expo'
            %% exponentials
        ...
        % fixed step time march (RK4)
            y_0 = eye(6); tspan = t(j:k); y_T = zeros(6,6);
            for i = 1:6
                [~,y] = RK4(@(t,y) model(t, y, ac, div_state, tspan), [tspan(1), tspan(end)], y_0(:,i), 1e-2);
                y_T(:,i) = y(end,:)';
            end
            FTM = y_T(1:3, 1:3);
