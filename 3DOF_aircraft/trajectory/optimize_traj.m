...
function [aircraft, sol] = optimize_traj(aircraft, x0, p)    
    ...
    options = optimoptions('fmincon', 'Display', 'none', 'Algorithm', 'sqp', 'MaxFunctionEvaluations', 2000000, 'MaxIterations', 10000);
    sol = fmincon(@(x) objfun(x, aircraft, 'traj'), xguess, [], [], [], [], lb, ub, @(x) constFun_traj(x, aircraft, 'test'), options);
