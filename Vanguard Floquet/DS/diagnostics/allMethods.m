...
[~,fG] = fourierdiff(60);
[~,freidmannFE] = freidmannMethod(trajData, fG);
[eigE,FE] = spectralMethod(trajData, true);

p1 = scatter(real(eigE), imag(eigE), 'xm');
hold on
p2 = scatter(FE, 0, 'sb');
p3 = scatter(real(freidmannFE), imag(freidmannFE), 75, 'or');
p4 = scatter(real(timeMarchFE), imag(timeMarchFE), 75, '*k');
legend([p2, p3, p4], {'Spectral', 'Freidmann', 'Time march'});
