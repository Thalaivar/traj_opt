...
%% Freidmann's method
fN = 50;
currFE = zeros(4,1); prevFE = Inf*ones(4,1);
Fdisc = [];
while(max(abs(real(currFE) - real(prevFE))) > 1e-5)
    prevFE = currFE;
    [~,freidmannGrid] = fourierdiff(fN);
    [~,currFE] = freidmannMethod(trajData, freidmannGrid);
    fN = fN + 2;
    Fdisc(end+1) = max(abs(real(currFE) - real(prevFE)));
end
freidmannPoints = fN; freidmannFE = real(sort(currFE, 'descend', 'ComparisonMethod', 'real'))';
[~,freidmannGrid] = fourierdiff(freidmannPoints);
tic
[~,currFE] = freidmannMethod(trajData, freidmannGrid);
freidMannTime = toc;
%% spectral method
fN = 50; Sdisc = [];
currFE = zeros(1,4); prevFE = Inf*ones(1,4);
while(max(abs(real(currFE) - real(prevFE))) > 1e-5)
    prevFE = currFE;
    [~,currFE] = spectralMethodMod(trajData, fN);
    fN = fN + 2;
    Sdisc(end+1) = max(abs(real(currFE) - real(prevFE)));
end
spectralPoints = fN; spectralFE = currFE;
tic
[~,currFE] = spectralMethodMod(trajData, spectralPoints);
spectralTime = toc;
...
while(max(abs(real(currFE) - real(prevFE))) > 1e-5)
    prevFE = currFE;
    [~,currFE] = timeMarchMethodMod(trajData, tol);
    tol = tol/10;
end
timMarchTol = tol; timeMarchFE = real(sort(currFE, 'descend', 'ComparisonMethod', 'real'))';

display(['The max disrepancy between Freidmann estimates and spectral estimates: ', num2str((max(abs(spectralFE - freidmannFE)))/mean(real(spectralFE)))])
display(['The max disrepancy between Freidmann estimates and time march estimates: ', num2str(max(abs(timeMarchFE - freidmannFE)))])
display(['The max disrepancy between time march estimates and spectral estimates: ', num2str(max(abs(spectralFE - timeMarchFE)))])
rmpath('../lib/')
%% functions
function [FTM,FE] = timeMarchMethodMod(trajData, tol)
    d = 4;
    Phi=zeros(d);
    Phi0=eye(d);
...
function dX = timeMarchModel(t, X, trajData)
    A = sysModel2(t, trajData);
    dX = A*X;
end
function A = sysModel2(t, trajData)
    prm.m = 4.5;
    prm.S = 0.473;
    prm.CD0 = 0.0173;
    prm.CD1 = -0.0337;
