...
% load solnData_A-0p02-0p8
% load solnData_A-0p02-0p2
% load solnData_A-0p11-0p6
% load solnData_A-0p08-0p8
% load solnData_A-0p01-0p95
load solnData_A-0p01-0p8
...
xlim([-limSet,limSet]);
title_str1 = horzcat('Glycolytic Floquent Exponents (a = ',num2str(solnData.A(1)),', b = ',num2str(solnData.A(2)),')');
...
% pp1.Title = horzcat('Periodic Eigenvectors (N = ',num2str(N),', ? = ',num2str(solnData.A),')'); 
...
function A = sysModel(t,solnData)
    ...
    A = [  -1+2*x1*x2,      solnData.A(1)+x1^2;
             -2*x1*x2, -(solnData.A(1)+x1*x1)];
end
