classdef aircraft
    %     3DOF aircraft object for
    %     used for 3DOF diff-flat diagnostics
    properties
        m
        b
        S
        CD0
        CD1
        CD2
        % normalization
        NCT
        
        p_exp
        VR
    end
    methods
        % constructor
        function obj = aircraft(m,b,S,CD0,CD1,CD2,NCT,p_exp,VR)
            obj.m = m;
            obj.b = b;
            obj.S = S;
            obj.CD0 = CD0;
            obj.CD1 = CD1;
            obj.CD2 = CD2;
            obj.NCT = NCT;
            obj.p_exp = p_exp;
            obj.VR = VR;
        end
        function dZ = stateDervs(obj,t,Z,splControls)
            %tvec used only if interp1 is used and not spline
            % Z = [V,chi,gam,x,y,z,CL,mu,CT]
            V = Z(1); chi = Z(2); gam = Z(3); % x =  Z(4); y = Z(5);
            z = Z(6);
            
            
            
%             CL = interp1(tvec,u(:,1),t);
%             mu = interp1(tvec,u(:,2),t);
%             CT = interp1(tvec,u(:,3),t)*obj.NCT;

            U= ppval(splControls,t);
            CL=U(1);
            mu=U(2);
            CT=U(3)*obj.NCT;
            
            dZ = zeros(6,1);
            
            %%%%% Wind Model
            % exponential
            % p_exp = 1;
            
            Wx = obj.VR*(-z)^obj.p_exp;
            Wxz = (obj.p_exp*obj.VR)*((-z)^obj.p_exp)/z;
            
            Cmu = cos(mu); Smu = sin(mu);
            Cgam = cos(gam); Sgam = sin(gam);
            Cchi = cos(chi); Schi = sin(chi);
            
            Q = 0.5*1.225*V*V;
            D = Q*obj.S*(obj.CD0 + obj.CD1*CL + obj.CD2*CL*CL);
            L = Q*obj.S*CL;
            T = Q*obj.S*CT;
            
            dZ(4) = V*Cchi*Cgam + Wx;
            dZ(5) = V*Schi*Cgam;
            dZ(6) = -V*Sgam;
            
            dZ(1) = -D/obj.m - 9.806*Sgam - Wxz*dZ(6)*Cchi*Cgam + T/obj.m;
            dZ(2) = L*Smu/(obj.m*V*Cgam) + Wxz*dZ(6)*Schi/(V*Cgam);
            dZ(3) = L*Cmu/(obj.m*V) - 9.806*Cgam/V + Wxz*Cchi*Sgam*dZ(6)/V;
        end
        function dy = linStateDervs(obj,t,y,uSpl,sSpl)   
            Z = ppval(sSpl,t);
 
            N = length(y);
            J = zeros(N);
            pert=1e-9;
            pertvar=pert*eye(N);
                for j=1:N
                    J(:,j)= 0.5*( obj.stateDervs(t,Z+pertvar(:,j),uSpl) - obj.stateDervs(t,Z-pertvar(:,j),uSpl) )/pert;
                end
            dy=J*y;
        end
    end
end